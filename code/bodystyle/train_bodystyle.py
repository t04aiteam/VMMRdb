...
# ///
"""Train a small bodystyle classifier (8 classes) on labels extracted from
boxcars116k_vmmr + stanfordcars_vmmr folder names, decoupled from the
make/model/year model.pt -- same reasoning as code/color/train_color.py:
only these two sources carry bodystyle ground truth, too narrow a slice to
justify a joint multi-task head.

Data: DATA/bodystyle_vmmr/<style_name>/*.jpg (built by
code/preprocess/bodystyle_labels.py).
Backbone: torchvision resnet18, ImageNet-pretrained, fc replaced with an
8-class head -- same choice as train_color.py (123k images / 8 classes,
resnet18 overfits less and trains fast enough on this machine).
Class imbalance (wagon/hatchback/van ~20-40k each, coupe/convertible/pickup
~1-1.3k each) handled with inverse-frequency loss weighting, not resampling
-- same approach as train_color.py.

Checkpoint format matches model.pt's / color_model.pt's own convention:
{"state_dict":..., "classes": [...]}.

Usage:
  uv run code/bodystyle/train_bodystyle.py --smoke   # fast correctness check
  uv run code/bodystyle/train_bodystyle.py           # full run
"""
import argparse
import random
import sys
...
sys.path.insert(0, str(__import__("pathlib").Path(__file__).resolve().parents[1] / "preprocess"))
...
DATA_ROOT = DATA / "bodystyle_vmmr"
OUT_CKPT = MAIN_REPO / "models" / "bodystyle_model.pt"

IMAGENET_MEAN = [0.485, 0.456, 0.406]
IMAGENET_STD = [0.229, 0.224, 0.225]


class BodystyleDataset(Dataset):
...
def main():
    ...
    if not classes:
        print(f"ERROR: no classes found under {DATA_ROOT} -- run code/preprocess/bodystyle_labels.py first.")
        return
    ...
    train_ds = BodystyleDataset(train, classes, train=True)
    val_ds = BodystyleDataset(val, classes, train=False)
    test_ds = BodystyleDataset(test, classes, train=False)
    ...
    # inverse-frequency class weights -- wagon/hatchback/van dominate ~30x over coupe/convertible/pickup
